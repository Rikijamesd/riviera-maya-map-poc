...
inventory tracker workbook. Unlike track-zisla-inventory.ps1 (which also
...
Usage:
  powershell -File download-zisla-payment-plans.ps1
  powershell -File download-zisla-payment-plans.ps1 -OutFile "D:\other\path.xlsx"
#>
param(
...
if ($planRows.Count -eq 0) {
  Write-Host "No payment plans found - nothing to write."
  return
}
...
try {
  ...
  $planSheet.UsedRange.Columns.AutoFit() | Out-Null

  $planTotalRows = $planSheet.UsedRange.Rows.Count - 1
